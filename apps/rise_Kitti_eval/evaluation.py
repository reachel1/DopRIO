...
class kittiOdomEval():
    def __init__(self, config):
        ...
        self.eval_seqs = [s[:-5] for s in self.eval_seqs]    # xxxx_pred => xxxx #xxxx_0x->xxxx
    ...
    def calcSequenceErrors(self, poses_gt, poses_result):
        ...
        dist = self.trajectoryDistances(poses_result)
        # every second, kitti data 10Hz
        self.step_size = 1
        # for all start positions do
        # for first_frame in range(9, len(poses_gt), self.step_size):
        for first_frame in range(0, len(poses_result), self.step_size):
            ...
            for i in range(self.num_lengths):
                ...
                pose_error = np.dot(np.linalg.inv(pose_delta_gt), pose_delta_result)
                r_err = self.rotationError(pose_error)
                t_err1 = self.translationError(pose_error)
                x_err2 = (poses_gt[last_frame][0,3]-poses_gt[first_frame][0,3])-(poses_result[last_frame][0,3]-poses_result[first_frame][0,3])
                y_err2 = (poses_gt[last_frame][1,3]-poses_gt[first_frame][1,3])-(poses_result[last_frame][1,3]-poses_result[first_frame][1,3])
                t_err2 = np.sqrt(x_err2**2+y_err2**2)
                if t_err2 > t_err1:
                    t_err = t_err1
                    state = 1
                else:
                ...
                if len_ == 300:
                    err_300.append(t_err/len_)
                    num_300.append(num)
                    state_300.append(state)
                    num = num + 1
        print("plot begin")
        plt.plot(num_300,err_300)       
        plt.show()
        return err
    ...
    def computeSegmentErr(self, seq_errs):
        ...
        for len_ in self.lengths:
            if segment_errs[len_] != []:
                ...
                print(len_)
                print("sequence translational mean:   {0:.3f}".format(avg_t_err))
            else:
    ...
    def eval(self, toCameraCoord):
        ...
        for seq in self.eval_seqs:
            ...
            poses_result = self.loadPoses(pred_file_name, toCameraCoord=False)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--gt_dir',     type=str, default='./gt',  help='Directory path of the ground truth odometry')
    parser.add_argument('--result_dir', type=str, default='./rise',              help='Directory path of storing the odometry results')
    parser.add_argument('--eva_seqs',   type=str, default='*',      help='The sequences to be evaluated') 
